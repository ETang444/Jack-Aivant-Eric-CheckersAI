...
public class JumpTree 
{
	...
	{
		System.out.println("We are now running fillInKids");
		...
		{
			System.out.println("The jump tree has detected that a piece can jump down-right.");
		...
		if(howWeGotHere.length()>5)
			System.out.println("Here is a node with a double jump chain. We built it.");
	...
	private void getAllJumpChainsHelp(JumpNode rt)
	{
		...
		if(rt.downRight==null&&rt.downLeft==null&&rt.upRight==null&&rt.upLeft==null)
		{
			...
			System.out.println("We are adding to the jump tree's allJumpChains: " + rt.howWeGotHere);
		}
		else
		{
